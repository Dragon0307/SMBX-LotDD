...
local boxColor = Color.black .. 0.5
...
local function newtextbox(args)
	...
	args.maxWidth = CheckForNil(args.maxWidth,boxWidth)
	...
	args.boxColor = CheckForNil(args.boxColor,boxColor)
	...
	obj.delay = defaultCharacterTicks
	obj.timer = defaultCharacterTicks
...
function Textbox:ResetVariables()
	...
	self.delay = defaultCharacterTicks
	self.timer = defaultCharacterTicks
...
function Textbox:Finish()
	...
	if not skipRunsTags then self.limit = nil end
...
function textbox.onMessageBox(eventToken, content, plr, NPC)
	...
	local x, y = 320 - boxWidth/2, 400

	if NPC then
		x = NPC.x + NPC.width/2 - boxWidth/2
		y = NPC.y - 50
	...
	for k,v in pairs(talkMessageProperties) do if not tbl[k] then tbl[k] = v end end
