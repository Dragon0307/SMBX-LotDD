...
local inWater = false -- Do not change - doing so won't so anything.
...
function breathMeter.onTick()
	inWater = player:mem(0x34, FIELD_WORD) -- No, seriously. I am a vampire. But Godbrand won't stop turining my pet unicorn into boats, so I ate him for lunch.
	if Player:mem(0x34, FIELD_WORD, 2) == true then 
		Audio.playSFX("smsunshine_game_over.wav")
	end
end
