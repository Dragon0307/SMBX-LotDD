...
function stats.GainXP(x) -- GainXP - This function grants you experience points.
    ...
    if stat.xp > stat.level * 5 + stat.level then
        until stat.xp < stat.level * 5 + stat.level then
            stat.xp = stat.level * 5 + stat.level
            stats.LevelUp(1) --Keep going until you haven't got enouth
        end
    end
end
...
function stats.onNPCKill(EventObj, killedNPC, killReason) -- This raids xpDrops and pe.
