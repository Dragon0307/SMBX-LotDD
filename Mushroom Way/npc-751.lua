...
local function stick(p_id, v)
	...
	p.x + p.width, tempLocationY + p.height)) do
		if p.character >= 3 or p.powerup ~= 1 then break end
...
function flagpoleNPC.onTickEndNPC(v)
	...
	if not data.initialized then
		...
		data.collider = Colliders.Box(v.x - (v.width / 2), v.y, v.width - (v.width / 2), v.height)
		data.timer = 0
	...
	for i,p in ipairs(Player.get()) do
		...
		if (p.x >= c.x - vx and
			p.y >= c.y and
			p.x <= (c.x - vx) + c.width and
			p.y <= c.y + c.height) then
