...
function attack.fireOrb()
    if attack.MPCheck(0) == true then
        ...
        NPC.spawn(13, player.x, player.y, player.section, false, true) --spawns a Yoshi fireball at your exact location.
        for index,myNPC in ipairs(NPC.get(13)) do
            if player.direction == 1 then -- npc.direction does not work with this NPC
                myNPC.speedX = 10
            elseif player.direction == 1 then -- npc.direction does not work with this NPC
                myNPC.speedX = -10
            end
        end
    end
...
function attack.chillout()
    if attack.MPCheck(0) == true then
        ...
        for index,myNPC in ipairs(NPC.get(237)) domyNPC.speedX = 4 * player.direction
        end
    end
end
