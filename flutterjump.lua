...
local jumptimer = 0
local fluttertimer = fluttertimer or 0
local canflutter = canflutter or true
local springed = springed or false
...
function flutterjump.onDraw()
	if canflutter and fluttertimer > 0 then
		player:mem(0x7A, FIELD_WORD, 7 + math.floor(lunatime.tick() * 0.4) % 3)
	end
end
